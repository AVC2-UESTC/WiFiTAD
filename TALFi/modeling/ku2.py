...
# from TALFi.modeling.atten import FullAttention, AttentionLayer, FullAttention2, AttentionLayer2
# from TALFi.modeling.encoder import Encoder, EncoderLayer, Encoder2, EncoderLayer2
...
#                                 activation_fn=None
#                             )
#         self.dw = nn.Conv1d(out_channels, out_channels, 3,1, 1, groups=out_channels)
#         self.maxdw = nn.Sequential(nn.MaxPool1d(3, 1, 1), nn.Conv1d(out_channels, out_channels, 3,1, 1, groups=out_channels))
#         self.lu = nn.SiLU(inplace=True)
#         self.nor = nn.GroupNorm(32, 512)
#         self.cross_attention = joint_attention(enc_in=out_channels, d_model=out_channels, n_heads=16, d_ff=out_channels*4, e_layers=1, factor=3, dropout=0.01, output_attention=False)

#     def forward(self, time):
#         time = self.Downscale_Blockt(time)
#         time = self.lu(self.nor(time + self.dw(time) + self.maxdw(time)))

#         time = self.cross_attention(time, time, time)
#         return time
...
#         self.conv = ConvBlock(512)
#         self.self_attention = joint_attention(enc_in=out_channels, d_model=out_channels, n_heads=16, d_ff=out_channels*4, e_layers=1, factor=3, dropout=0.01, output_attention=False)
#         self.cross_attention = joint_attention(enc_in=out_channels, d_model=out_channels, n_heads=16, d_ff=out_channels*4, e_layers=1, factor=3, dropout=0.01, output_attention=False)
#     def forward(self, time):
#         time = self.Downscale_Blockt(time)

#         high =self.conv(time)
#         low = self.self_attention(time, time, time)

#         out = self.cross_attention(low, high, high)
#         return out
...
class TFBlock(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=2, stride=2):
        ...
        self.conv = ConvBlock(512)
        self.self_attention = joint_attention(enc_in=out_channels, d_model=out_channels, n_heads=16, d_ff=out_channels*4, e_layers=1, factor=3, dropout=0.01, output_attention=False, attn='full')
        self.cross_attention = joint_attention(enc_in=out_channels, d_model=out_channels, n_heads=16, d_ff=out_channels*4, e_layers=1, factor=3, dropout=0.01, output_attention=False, attn='cross')
    def forward(self, time):
        time = self.Downscale_Blockt(time)
