...
from TALFi.modeling.ku2 import Unit1D,ScaleExp, Transformer_encoder
from TALFi.modeling.atten import FullAttention, AttentionLayer, FullAttention_new, Cross_Attention
from TALFi.modeling.encoder import Encoder, EncoderLayer, Encoder2, EncoderLayer2, Encoder3, EncoderLayer3
import math
...
class Rec(nn.Module):
    def __init__(self):
        ...
        self.d1 = nn.Sequential(nn.Linear(512,512, bias=True), nn.Tanh())
        self.d2 = nn.Sequential(nn.Linear(512,512, bias=True), nn.Tanh())
    def forward(self, x, memory):
        x_channel = self.squeeze(x).squeeze(-1)
        memory = memory * self.d1(x_channel)
        x_new = x * (self.d2(memory)).unsqueeze(-1)
        return  x_new
...
class GaussianSampling(nn.Module):
    def __init__(self, len,r):
        ...
        # self.crs1 = joint_attention(length=1, enc_in=512, d_model=512, n_heads=16, d_ff=512*4, e_layers=1, factor=3, dropout=0.01, output_attention=False, attn='full2')
        # self.crs2 = joint_attention(length=1, enc_in=512, d_model=512, n_heads=16, d_ff=512*4, e_layers=1, factor=3, dropout=0.01, output_attention=False, attn='full2')
        # self.crs3 = DeformableAttention1D(
        #             dim = len,
        #                         downsample_factor = 1,
        #                         offset_scale = 1,
