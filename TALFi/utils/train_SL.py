# bash TALFi/train_tools/SlideWindow.sh 3
import os
import random
import tqdm
import numpy as np
...
from TALFi.dataset.Classify_dataloader import SmartWiFi, get_video_info, \
    load_video_data, detection_collate, get_video_anno
from torch.utils.data import DataLoader
from TALFi.modeling.meta_model2 import SLmodel
from TALFi.modeling.multi_losses import Classification_loss
...
def forward_one_epoch(net, clips, targets, training=True):
    ...
    loss_c = CPD_Loss(output_dict['conf'], targets)
    return loss_c
...
def run_one_epoch(epoch, net, optimizer, data_loader, epoch_step_num, training=True):
    ...
    with tqdm.tqdm(data_loader, total=epoch_step_num, ncols=0) as pbar:
        for n_iter, (clips, targets) in enumerate(pbar):
            iteration = n_iter
            loss_c = forward_one_epoch(net, clips, targets, training=training) * config['training']['cw']
            cost = loss_c
            if training:
                optimizer.zero_grad()
                cost.backward()
    ...
    plog = 'Epoch-{} {} Loss: Total - {:.5f}, conf - {:.5f}'\
        .format(epoch, prefix, cost_val, loss_conf_val)
    print(plog)
...
if __name__ == '__main__':
    ...
    CPD_Loss = Classification_loss(num_classes, use_focal_loss=focal_loss)
